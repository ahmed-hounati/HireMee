...
{{--            ////////////////////--}}
            ...
            <div class="w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
                @if ($offer->image)
                    <img class="object-cover object-center w-full h-56" src="{{ asset('images/' . $offer->image) }}" alt="Employment Image">
                @endif

                <div class="flex items-center px-6 py-3 bg-gray-900">

                    <h1 class="mx-3 text-lg font-semibold text-white">{{$offer->title}}</h1>
                </div>

                <div class="px-6 py-4">
                    <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Patterson johnson</h1>

                    <p class="py-2 text-gray-700 dark:text-gray-400">{{$offer->description}}</p>
                    @foreach(json_decode($offer->competences) as $compt)
                    ...
                    <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                        ...
                        <h1 class="px-2 text-sm">{{$offer->contract}}</h1>
                    </div>
{{--                        adress--}}
                    <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                        ...
                        <h1 class="px-2 text-sm">{{$offer->emplacement}}</h1>
                    </div>
                    ...
                    <div class="flex justify-between mt-4">
                        ...
                        <form method="post" action="{{$offer->id}}">
                            @csrf
                            @method('DELETE')
                            <button class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600"><i
            ...
            <a href="/entreprise/emplois/create"
